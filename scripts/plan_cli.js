#!/usr/bin/env node
// usage: node scripts/plan_cli.js "Arnavutköy" "Üsküdar" [YYYY-MM-DD] [HH:MM] [--window 240] [--walk] [--xfer 3]
...
const js = P.plan(db, A, B, date, h * 60 + m, { windowMin, transferMin, allowWalk: walk });
...
for (const j of js) {
  ...
  j.legs.forEach((l, i) => {
    ...
    else console.log(`   ⛴  ${P.fmt(l.dep)}${l.depEst ? '~' : ''} ${l.from} → ${P.fmt(l.arr)}${l.arrEst ? '~' : ''} ${l.to}   [${l.line.name}]${l.trip.note ? '  ' + l.trip.note : ''}`);
  });
  console.log();
}
